...
parser.add_argument("--model_name_or_path", type=str, default="output",
                    help="path to pre-trained model or shortcut name")
parser.add_argument("--input", type=str,
                    help="input text file, each line corresponding to one instance")
parser.add_argument("--output", type=str, help="output file")
parser.add_argument("--eos_token_id", type=int,
                    default=None, help="eos token id")
...
for batch in tqdm(range(args.jobid, len(prompts), batch_size * args.jobnum)):
    ...
    encodings_dict = tokenizer.batch_encode_plus(
        prompt_text, max_length=None, pad_to_max_length=True)
    ...
    start_idx = inp_idx = (last_non_masked_idx).view(-1, 1).repeat(
        1, tokenizer.vocab_size + len(tokenizer.additional_special_tokens)).unsqueeze(1)
    past = None

    position_ids = torch.tensor([list(range(seq_len))
                                for i in range(input_ids.shape[0])])
    for i, position_ids_slice in enumerate(position_ids):
        position_ids_slice[last_non_masked_idx[i]
            :] = position_ids_slice[last_non_masked_idx[i]]
    ...
    for step in range(num_tokens_to_produce):
        ...
        attn_mask = torch.cat([attn_mask, torch.ones(
            (attn_mask.shape[0], 1)).long().to(args.device)], dim=1)
    ...
    ret += [tokenizer.decode(output, skip_special_tokens=False,
                             clean_up_tokenization_spaces=True).replace("<|endoftext|>", "") for output in input_ids]
